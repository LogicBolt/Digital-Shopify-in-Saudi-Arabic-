...
class VariantSelects extends HTMLElement {
  ...
  updateMedia() {
    ...
    if (mediaGallery && this.currentVariant && this.currentVariant.featured_media && this.currentVariant.featured_media.id) {
      mediaGallery.setActiveMedia(`${this.dataset.section}-${this.currentVariant.featured_media.id}`, true);
    } else {
      //console.error('Unable to set active media. Some required variables are not defined.');
    }
   // mediaGallery.setActiveMedia(`${this.dataset.section}-${this.currentVariant.featured_media.id}`, true);
/*
...
class LocalizationForm extends HTMLElement {
  ...
  hidePanel() {
    this.elements.button.setAttribute('aria-expanded', 'false');
     this.elements.popup.style.display = 'none'; // Hide the popup
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  disclosureButton.addEventListener('click', function () {
    dropupContent.classList.toggle('open');
  });

  // Close the dropdown when clicking outside
  document.addEventListener('click', function (event) {
    if (!dropupContent.contains(event.target) && !disclosureButton.contains(event.target)) {
      dropupContent.classList.remove('open');
    }
  });
});

/***** End Language and Currency switcher */